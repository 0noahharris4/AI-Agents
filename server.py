	...
	if openai_api_key:
		...
		if st.button("Analyze Log with AI"):
			...
			try:
				response = openai.chat.completions.create(
					...
					messages=[
						{"role": "system", "content": (
							...
							"\n\n1. An initial short inference (1-2 sentences) about what is most likely causing the error, labeled 'Inference'. "
				...
				st.markdown(f"<div style='background: rgba(34,34,34,0.7); color: #00ffe7; padding: 10px 16px; border-radius: 8px; font-size: 1.08rem; margin-bottom: 10px;'><b>Inference:</b> {inference if inference else 'No inference found.'}</div>", unsafe_allow_html=True)
